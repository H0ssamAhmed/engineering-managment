...
export const useProjects = () => {
  ...
  const createProject = useCallback((projectData: Omit<Project, "id" | "created_at" | "updated_at" | "status">, userId: string) => {
  ...
  const updateStage = useCallback((projectId: string, stageId: string, updates: Partial<ProjectStage>, userId: string) => {
    const now = new Date().toISOString();
    const today = now.split("T")[0];

    setStages((prevStages) => {
      const projectStages = prevStages.filter((s) => s.project_id === projectId).sort((a, b) => a.stage_order - b.stage_order);
      const currentStageIndex = projectStages.findIndex((s) => s.id === stageId);
      ...
      let finalStages = prevStages.map((s) => (s.id === stageId ? updatedStage : s));
      ...
      if (updates.status === "completed") {
        ...
        if (nextStage && nextStage.status === "not_started") {
          ...
          setProjects(prev => prev.map(p => 
            p.id === projectId ? { ...p, current_stage_id: nextStage.id, updated_at: now } : p
          ));
          ...
          setProjects(prev => prev.map(p => 
            p.id === projectId ? { ...p, status: "completed", updated_at: now } : p
          ));
  ...
  const stats = {
    ...
    stageDistribution: stages.reduce((acc, stage) => {
